import IconFile from '@components/atoms/Icons/IconFile';
...
const Table: React.FC<TableProps> = ({ filterItems, data, columns }) => {
  ...
  useEffect(() => {
    setInitialRecords(() => {
      return data.filter((item) => {
        return filterItems.some((filter) => item[filter].toString().toLowerCase().includes(search.toLowerCase()));
      });
    });
  ...
  const body: { [key: string]: string | number | boolean }[] | (string | number | boolean)[][] = data.map((item) =>
      ...
      <div className="mt-6 panel">
        <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
          <div className="flex flex-wrap items-center">
            <button type="button" onClick={() => exportTable('csv')} className="m-1 btn btn-primary btn-sm ">
              <IconFile className="w-5 h-5 ltr:mr-2 rtl:ml-2" />
              CSV
            </button>
            <button type="button" onClick={() => exportTable('txt')} className="m-1 btn btn-primary btn-sm">
              <IconFile className="w-5 h-5 ltr:mr-2 rtl:ml-2" />
              TXT
            </button>

            <button type="button" className="m-1 btn btn-primary btn-sm" onClick={handleDownloadExcel}>
              <IconFile className="w-5 h-5 ltr:mr-2 rtl:ml-2" />
              EXCEL
            </button>

            <button type="button" onClick={() => exportTable('print')} className="m-1 btn btn-primary btn-sm">
